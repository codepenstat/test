...
<script>
    ...
    const rows2 = document.querySelectorAll('#binStatsTable tbody tr');
    rows2.forEach(row => {
...
<script>
    // Выделение всей таблицы по двойному клику
    document.getElementById('zipStatsTable').addEventListener('dblclick', function() {
    ...
    // Выделение строки при одиночном клике
    const rows3 = document.querySelectorAll('#zipStatsTable tbody tr');
    rows3.forEach(row => {
        row.addEventListener('click', function(event) {
            const selected = this.classList.toggle('selected');
            if (selected) {
...
<script>
    ...
    // Выделение строки при одиночном клике
    const rowsFiltered2 = document.querySelectorAll('#filteredZipStatsTable tbody tr');
    rowsFiltered2.forEach(rowFiltered => {
        rowFiltered.addEventListener('click', function(event) {
            const selectedFiltered = this.classList.toggle('selected');
            if (selectedFiltered) {
                this.style.backgroundColor = '#d1e7dd'; // Цвет выделенной строки
